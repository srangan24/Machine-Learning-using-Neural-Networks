...
import functions.ActivationImplementation;
...
public class Networking {
	...
	private ActivationImplementation function;
	private Sigmoid sigmoid;
	
	// constructor that initializes variables based on sample data and sample results
	public Networking(float[][] inputs, int[] output) {
		/*
		...
		public void run() {
			...
			for (int i = 0; i <= iterations; i++) {
				...
				for (int j = 0; j < outputSize; j++) {
					...
					calculate = new Calculate(getRow(j), weight_1, bias, weight_2, neurons, sigmoid);
